...
-- Schema ProciDataBase
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `ProciDataBase` ;
USE `ProciDataBase` ;
...
CREATE TABLE IF NOT EXISTS `ProciDataBase`.`cuenta` (
  ...
  `contraseña` VARCHAR(45) CHARACTER SET 'utf8mb4' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ProciDataBase`.`salida` (
  ...
  CONSTRAINT `material`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `unidad_salida`
    FOREIGN KEY (`id_unidad`)
    REFERENCES `ProciDataBase`.`inventario` (`unidad`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_usuario_salida`
